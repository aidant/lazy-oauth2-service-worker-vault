...
const OAUTH2_CLIENT_ID = new URLSearchParams(location.search).get('oauth2_client_id') || ''
const OAUTH2_TOKEN_URL = new URLSearchParams(location.search).get('oauth2_url_token') || ''
// @ts-expect-error URLPattern does not yet have types.
const OAUTH2_PROTECTED_RESOURCE_URL = new URLPattern(
  Object.fromEntries(
    [...new URLSearchParams(location.search).entries()]
      .filter(([key]) => key.startsWith('oauth2_url_protected_resource_'))
      .map(([key, value]) => [key.replace('oauth2_url_protected_resource_', ''), value])
  )
)
...
export const modifyRequest = (request: Request): Request => {
...
export const modifyResponse = async (response: Response): Promise<Response> => {
